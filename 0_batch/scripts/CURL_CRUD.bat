...
%CURL% -d @%QUERY_DIR%/created_department.json -X POST "%SITE%/departments?%REPO_TYPE%"
@echo.
@powershell -Command Write-Host "CREATE employee" -foreground "Green"
%CURL% -d @%QUERY_DIR%/created_employee.json -X POST "%SITE%/employees?%REPO_TYPE%"
@echo.
...
%CURL% -d @%QUERY_DIR%/updated_department.json -X PATCH "%SITE%/departments/%DEP_ID%?%REPO_TYPE%"
@echo.
@powershell -Command Write-Host "UPDATE employee by id" -foreground "Green"
%CURL% -d @%QUERY_DIR%/updated_employee.json -X PATCH "%SITE%/employees/%EMP_ID%?%REPO_TYPE%"
@echo.
...
%CURL% "%SITE%/departments/%DEP_ID%?%REPO_TYPE%"
@echo.
@powershell -Command Write-Host "READ employee by id - after UPDATE" -foreground "Green"
%CURL% "%SITE%/employees/%EMP_ID%?%REPO_TYPE%"
@echo.&pause
...
%CURL% -X DELETE "%SITE%/employees/%EMP_ID%?%REPO_TYPE%"
@echo.
@powershell -Command Write-Host "DELETE department by id" -foreground "Green"
%CURL% -X DELETE "%SITE%/departments/%DEP_ID%?%REPO_TYPE%"
@echo.
