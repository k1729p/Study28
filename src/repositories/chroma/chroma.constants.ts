...
/**
 * A flat, primitive-only record. This is the shape Chroma requires for
 * collection metadata (string | number | boolean values only - no nested
 ...
 * structured, non-semantic-search nature of this dataset. The collections
 * are created with `embeddingFunction: null`, so Chroma requires callers to
 * supply an embedding explicitly on every add/upsert - this function is that
 * supplier.
 * @param text the text to derive a vector from (e.g. an employee's full name)
 ...
 * Optional fields are omitted entirely (rather than set to null/undefined) since
 ...
 * `departmentId` is always included so employees can be grouped back under
 * their department when reading (Chroma's equivalent of the SQL foreign key).
